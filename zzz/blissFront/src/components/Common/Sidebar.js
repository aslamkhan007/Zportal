...
import { createProjectAction, editProgramAction } from "../../redux/Action";
import { NotificationContainer } from "react-notifications";
import CreateProgram from "../Programs/CreateProgram";
import ProgramPopupModal from "../Project/popupModel/ProgramPopupModal";
import ProjectPopupModal from "../Project/popupModel/ProjectPopupModal";
...
const Sidebar = (props) => {
  ...
  const [showProjectModalPopup, setShowProjectModalPopup] = useState(false);
  const [showImportModalPopup, setShowImportModalPopup] = useState(false);
  const [showEditProjectModalPopup, setShowEditProjectModalPopup] =
    useState(false);
  ...
  const [showConfirmProgramPopup, setShowConfirmProgramPopup] = useState(false);
  const [showDeleteConfirmProgramPopup, setShowDeleteConfirmProgramPopup] =
    useState(false);
  const [programs, setPrograms] = useState([]);
  const [projectName, setProjectName] = useState("");
  const [projectDescription, setProjectDescription] = useState("<p></p>");
  ...
  const [programDescription, setProgramDescription] = useState("<p></p>");
  ...
  const isShowProgramPopup = (status) => {
    setShowProgramPopup(status);
    setProgramName("");
    setProgramDescription("");
  };
  const isShowEditProgramPopup = (status, id = null, obj = {}) => {
    setShowEditProgramPopup(status);
    if (id !== null) {
      setProgramId(id);
      setProgramName(obj.name);
      setProgramDescription(obj.description);
      setProgramObj(obj);
    }
  };
  const isShowConfirmProgramPopup = (status, id) => {
    setShowConfirmProgramPopup(status);
    setProgramId(id);
  };
  const isShowDeleteConfirmProgramPopup = (status, id) => {
    setShowDeleteConfirmProgramPopup(status);
    setProgramId(id);
  };

  const isShowConfimPopup = (status, id) => {
    setShowConfirmPopup(status);
    setProjectId(id);
  };
  const isShowDeleteConfimPopup = (status, id) => {
    setShowDeleteConfirmPopup(status);
    setProjectId(id);
  };

  const isShowProjectPopup = (status, id) => {
    setProjectName("");
    setProjectDescription("");
    setShowProjectModalPopup(status);
    setProgramId(id);
  };
  const isShowImportPopup = (status) => {
    setShowImportModalPopup(status);
  };
  const isShowEditProjectPopup = (
    status,
    id,
    projectName,
    description,
    ...
    setShowEditProjectModalPopup(status);
    setProjectId(id);
  ...
  const toggle = () => {
    let status = !addClass ? true : false;
    setAddClass(status);
  };
  ...
  useEffect(() => {
    if (props.openProject) {
      setShowProjectModalPopup(true);
    }
    if (props.openImport) {
      setShowImportModalPopup(true);
    }
  }, [props]);
  ...
  if (
    props.sidebar &&
    props.sidebar.sidebarSubTitle &&
    props.sidebar.sidebarSubTitle[1].link === "account" &&
    props.sidebar.sidebarSubTitle[2].link === "user" &&
    props.sidebar.sidebarSubTitle[3].link === "security" &&
    props.sidebar.sidebarSubTitle[4].link === "recycle" &&
    props.sidebar.sidebarSubTitle[5].link === "archive-page" &&
    props.sidebar.sidebarSubTitle[6].link === "import"
  ) {
  ...
  const setProjectDes = (e) => {
    console.log("des:::", e);
    setProjectDescription(e);
  };

  const setEditProject = (e) => {
    setEditProjectName(e.target.value);
  };
  const setEditProjectDesc = (e) => {
    setEditProjectDescription(e);
  };
  ...
  const onProjectCreateSubmit = (e, collapse) => {
      ...
      createProjectAction(data)
        .then((result) => {
          ...
          if (result.status === 200) {
            ...
            setShowProjectModalPopup(false);
            listPrograms();
          } else {
  ...
  const isValidEditProject = (e) => {
    let error = {};
    let formIsValid = true;

    if (editProjectName === "" || editProjectName.trim().length === 0) {
      formIsValid = false;
      error["editProjectName"] = "Please enter project name";
    }
  ...
  const onProjectEditSubmit = (e, collapse) => {
    console.log("in onProjectEditSubmit");
    setLoading(true);
    if (isValidEditProject()) {
      const data = {
    ...
    editProjectAction(data)
      .then((result) => {
        ...
        if (result.status === 200) {
          ...
          setShowEditProjectModalPopup(false);
          listPrograms();
        } else {
          console.log("in error");
          createNotification("error", result.message);
  ...
  const onDeleteConfirm = () => {
    console.log("delete confirm", projectId);
    const data = {
      projectId: projectId,
  ...
  const setDescription = (data) => {
    console.log("edor data:::", data);
    setProgramDescription(data);
  };

  const isValidCreateProgram = (e) => {
    let error = {};
    let formIsValid = true;

    if (programName === "" || programName.trim().length === 0) {
      formIsValid = false;
      error["programName"] = "Please enter program name";
    }
    if (programDescription === "" || programDescription.trim().length === 0) {
      formIsValid = false;
      error["programDescription"] = "Please enter program description";
    }

    setCreateProgramErrors(error);
    return formIsValid;
  };

  const onProgramSubmit = (e, collapse) => {
    console.log("in onProgramEditSubmit");
    setLoading(true);
    if (isValidCreateProgram()) {
      const workspaceId =
        workspaceState &&
        workspaceState.selectedWorkspace &&
        workspaceState.selectedWorkspace._id
      ...
      setLoading(false);
      console.log("errors", createProgramErrors);
    }
  };
    ...
    editProgramAction(data)
      .then((result) => {
        ...
        if (result.status === 200) {
          ...
          isShowConfirmProgramPopup(false);
          isShowDeleteConfirmProgramPopup(false);
          listPrograms();
        } else {
          console.log("in error");
          createNotification("error", result.message);
  ...
  const onEditProgramSubmit = (e, collapse) => {
    console.log("in onProgramEditSubmit");
    setLoading(true);
    if (isValidCreateProgram()) {
      const data = {
  ...
  return (
    <>
      <button className="menuOpen" onClick={toggle.bind(this)}>
        <svg
          ...
          <ProSidebar>
            <h3 className="d-flex align-items-center justify-content-between sofia-semibold mb-3 ">
              ...
              <svg
                ...
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                onClick={() => isShowProgramPopup(true)}
              >
                <path
                  fill-rule="evenodd"
                  clip-rule="evenodd"
              ...
              {/* <CreateProject
                showModalPopup={showProjectModalPopup}
                onPopupClose={isShowProjectPopup}
                onOpenImport={isShowImportPopup}
                onProjectCreateSubmit={onProjectCreateSubmit}
                projectName={projectName}
              ...
              <ProjectPopupModal
                ...
                onProjectEditSubmit={onProjectEditSubmit}
                errors={errors}
                titleText="CREATE PROJECT"
                type="add"
              />
              <ProjectPopupModal
                showModalPopup={showEditProjectModalPopup}
                onPopupClose={isShowEditProjectPopup}
                onProjectCreateSubmit={onProjectCreateSubmit}
                projectName={editProjectName}
                projectDescription={editProjectDescription}
                setProjectDes={setEditProjectDesc}
              ...
              <ConfirmBox
                ...
                onPopupClose={isShowConfimPopup}
                onConfirm={onConfirm}
                warningText={
                  <>
                    <p>You are about to Archive the selected project .</p>
              ...
              <ConfirmBox
                ...
                onPopupClose={isShowDeleteConfimPopup}
                onConfirm={onDeleteConfirm}
                warningText={
                  <>
                    <p>
                      You are about to permenantly delete ALL items from the
                      recycle bin.{" "}
                    </p>
                    <p>Please confirm this is the action you wish to take</p>
                  </>
              ...
              />
              <ProgramPopupModal
                showModalPopup={showProgramPopup}
                onPopupClose={isShowProgramPopup}
                onProgramSubmit={onProgramSubmit}
                ...
                titleText="NEW PROGRAM"
                btnText="Add"
                type="add"
              />
              <ProgramPopupModal
                showModalPopup={showEditProgramPopup}
                onPopupClose={isShowEditProgramPopup}
                onProgramSubmit={onEditProgramSubmit}
                programName={programName}
                programDescription={programDescription}
                setProgram={setProgram}
                programObj={programObj}
            ...
            /> */}
              ...
              /> */}
              <ConfirmBox
                showConfirmPopup={showConfirmProgramPopup}
                onPopupClose={isShowConfirmProgramPopup}
                onConfirm={onProgramArchiveConfirm}
                warningText={
                  <>
                    <p>You are about to archive program.</p>
                    <p>Please confirm this is the action you wish to take</p>
                  </>
                }
              />
              <ConfirmBox
                showConfirmPopup={showDeleteConfirmProgramPopup}
                onPopupClose={isShowDeleteConfirmProgramPopup}
                onConfirm={onProgramDeleteConfirm}
                warningText={
                  <>
                    <p>You are about to delete program.</p>
                    <p>Please confirm this is the action you wish to take</p>
                  </>
                }
              />
            ...
            <ul className="list-unstyled child-bar-list">
              ...
              typeof props.sidebar.userBoard != "undefined" &&
              props.sidebar.userBoard ? (
                typeof props.sidebar.sidebarSubTitle != "undefined" ? (
                  props.sidebar.sidebarSubTitle.map((sub, index_p) => (
                ...
                <>
                  {programs &&
                    programs.length > 0 &&
                    programs.map((data, index_c) => (
                      <li key={"menu_p" + index_c}>
                        <Menu>
                          <MenuItem>
                            <div className="child-bar-list-item d-flex align-items-center justify-content-between">
                              <span
                              ...
                              <svg
                                ...
                                xmlns="http://www.w3.org/2000/svg"
                                onClick={() =>
                                  isShowProjectPopup(true, data._id)
                                }
                              >
                                <path
                                  fill-rule="evenodd"
                              ...
                              <Dropdown>
                                ...
                                <Dropdown.Menu align="right">
                                  ...
                                  <Dropdown.Item
                                    onClick={() =>
                                      isShowConfirmProgramPopup(true, data._id)
                                    }
                                  >
                                    Archive
                                  </Dropdown.Item>
                                  <Dropdown.Item
                                    onClick={() =>
                                      isShowDeleteConfirmProgramPopup(
                                        true,
                                        data._id
                                      )
                                    }
                          ...
                          {data.projects && data.projects.length > 0 ? (
                            <SubMenu
                              title={data.name}
                              defaultOpen={index_c === 0 ? true : false}
                            >
                              <ul className="list-unstyled ff">
                                {data.projects.map((child, index) => (
                                  <li key={"menu_c" + child._id}>
                                    <div className="d-flex align-items-center justify-content-between">
                                      <Link to={`/programs/${child._id}`}>
                                        <span
                                          onMouseOver={() => hoverOn()}
                                          onMouseOut={() => hoverOut()}
                                        >
                                          ...
                                          {/* {child.projectName .length > 2 ? data.name.slice(0,2) + "..." :  child.projectName } */}
                                          {checkHover
                                            ? child.projectName
                                            : child.projectName.length > 12
                                            ? child.projectName.slice(0, 12) +
                                              "..."
                                            : child.projectName}
                                        </span>

                                        {/* {console.log(child.projectName.length,"hhhhhhhhhhhhhh")}  */}
                                      </Link>
                                      <Dropdown>
                                        <Dropdown.Toggle className="p-0 bg-transparent border-0">
                                          <svg
                                        ...
                                        <Dropdown.Menu align="right">
                                          ...
                                          <Dropdown.Item
                                            onClick={() =>
                                              isShowConfimPopup(true, child._id)
                                            }
                                          >
                                            Archive
                                          </Dropdown.Item>
