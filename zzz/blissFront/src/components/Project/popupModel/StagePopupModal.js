...
import { createTicketAction, getInvitedWorkspaceUsersAction, editTicketAction, getWorkspaceAction } from "../../../redux/Action";
...
const ModalPopup = (props) => {
    ...
    const displayEditData = () => {
        if (props.stageObj) {
            let { assignTo, startDate, dueDate, priority, watcher, estimate, workedOn, progress, riskStatus } = props.stageObj
            ...
            const collapseState = {
                assignTo,
                startDate: startDate ? moment(new Date(startDate)).toDate() : moment(new Date()).toDate(),
                dueDate: dueDate ? moment(new Date(dueDate)).toDate() : moment(new Date()).toDate(),
                priority: priority,
                watcher: watcher,
                estimate: estimate,
                workedOn: startDate ? moment(new Date(workedOn)).toDate() : moment(new Date()).toDate(),
                progress: progress,
    ...
    const [stageDescription, setStageDescription] = useState("<p></p>")
    ...
    useEffect(() => {
        ...
        if (props.type !== 'add') {
            ...
            //     setStageDescription(props.description)
            // }
        } else {
            setStageName("")
            setStageDescription("<p> </p>")
        }
        if (descriptionInstance && props.description) {
            // descriptionInstance.eval(description);
            // description.setData(description);

            descriptionInstance.setData(props.description);
        }

    }, [props.name, descriptionInstance])
                        ...
                        <span className="row">
                            <span className="col-xl-9">
                                ...
                                <Form.Group className="mb-2 col-xl-12 " controlId="formUser">
                                    <Form.Label className="ticket-light">STAGE NAME</Form.Label>
                                    <Form.Control
                                        type="text"
                                        placeholder=""
                                        value={stageName}
                                ...
                                <Form.Group className="mb-2 col-xl-12 " controlId="formUser">
                                    ...
                                    <CKEditor
                                        initData={stageDescription}
                                        onChange={({ editor }) => {
                                            let htmlString = editor.getData();
                                            setStageDescription(htmlString);
                                        }}
