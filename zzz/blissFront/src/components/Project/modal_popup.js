...
import toggleicon from "./../../assets/images/popuptoggleicon.png";
import {
  createNotification,
  // getFileObject,
...
import {
  createTicketAction,
  getInvitedWorkspaceUsersAction,
  editTicketAction,
  getWorkspaceAction,
} from "../../redux/Action";
...
const initialSubtasks = [{ title: "", description: "", complete: false }];
...
// const initialStatus = [
//   { id: 1, status: "BACKLOG" },
//   { id: 2, status: "THIS SPRINT" },
//   { id: 3, status: "IN PROGRESS" },
//   { id: 4, status: "IN REVIEW" },
//   { id: 5, status: "DONE" }
// ]
...
const ModalPopup = (props) => {
  ...
  const [statusOptions, setStatusOptions] = useState(props.project.status);
    ...
    try {
      ...
      if (result.data.status === 200) {
        ...
        if (
          ...
          const res = await getInvitedWorkspaceUsersAction(
            workSpaceObj.workSpace._id
          );
          const { results } = res.data;
          if (results && results.length) {
            const options = results.map((val) => {
  ...
  const [subTaskArr, setSubTaskArr] = useState(initialSubtasks);

  useEffect(() => {
    setStatusOptions(props.project.status);
    loadData();
  }, [props.project]);
  const isValid = (e) => {
    let error = {};
    ...
    if (selectedStatus === "" || selectedStatus.trim().length === 0) {
      formIsValid = false;
      error["selectedStatus"] = "Please select a status";
    }
  ...
  const onSubmit = (e) => {
    ...
    if (isValid(e)) {
      ...
      // const attachments = images.map((val) => val.file);
      ...
      collapse["assignTo"] = collapse.assignTo.length
        ? collapse.assignTo.map((v) => v.value)
        : [];
      ...
      if (props.type && props.type !== "edit") {
        ...
        Object.entries(data).forEach((val) => {
          let jsonObj = JSON.stringify(val[1]);

          formData.append(val[0], jsonObj);
        });
  ...
  const loadTicket = async () => {
    console.log("currentTicket=", props.currentTicket);
    if (props.currentTicket) {
      ...
      if (AssignOptions && AssignOptions.length) {
        assignTo =
          assignTo && assignTo.length
            ? assignTo.map((val) => {
              const temp = AssignOptions.find((v) => {
                return v.value === val;
              });
      ...
      let imageList =
        attachments && attachments.length
          ? await getFileObjectFromUrl(attachments)
          : [];
      // ? attachments.map((img) => {
      ...
      //     return {
      //       file: new File([url], name, { type: `image/${ext}` }),
      //       data_url: url,
      //     };
      //   })
      // : [];

      // imageList = await getFileObjectFromUrl(imageList);

      // console.log("listing ", imageList);

      // const blobs = await getFileObjectFromUrl(imageList);
      ...
      //         return {
      //           data_url: `${CONFIG.API_URL}/uploads/attachments/${v}`,
      ...
      let collapse = {
        ...
        dueDate: props.currentTicket.dueDate
          ? moment(new Date(props.currentTicket.dueDate)).toDate()
          : "",
        estimate,
        ...
        startDate: props.currentTicket.startDate
          ? moment(new Date(props.currentTicket.startDate)).toDate()
          : "",
        workedOn: props.currentTicket.workedOn
          ? moment(new Date(props.currentTicket.workedOn)).toDate()
          : "",
        watcher,
      };

      if (descriptionInstance) {
        // descriptionInstance.eval(description);
        // description.setData(description);

        descriptionInstance.setData(description);
      }
      setSelectedStatus(status);
      setCollapseState(collapse);
      setSubTaskArr(subTasks);
      setStage(props.currentTicket.stageId);
      // console.log("descr", props.currentTicket.description);
      setDescription(props.currentTicket.description);

      setTicketName(props.currentTicket.title);
      setImages(imageList);
      setTicketId(props.currentTicket._id);
    }
  };

  useEffect(() => {
    loadTicket();
  }, [props.currentTicket, AssignOptions.length, descriptionInstance]);
  // useEffect(()=>{
      ...
      >
        <Modal.Header closeButton>
          ...
          <tr className="Header_top">
            <img className="popup-micon" src={toggleicon} />
          ...
          <Form onSubmit={(e) => onSubmit(e)}>
            <span className="row">
              <span className="col-xl-9">
                ...
                <Form.Group className="mb-2 col-xl-12 " controlId="formUser">
                  <Form.Label className="ticket-light">STATUS</Form.Label>
                  <Form.Control
                    aria-label="Default select example"
                    as="select"
                  ...
                  >
                    ...
                    {statusOptions &&
                      statusOptions.map((data) => (
                        <option value={data._id}>{data.status}</option>
                      ))}
                  </Form.Control>
                  <p className="error_mesage"> {errors.selectedStatus} </p>
                </Form.Group>
                <Form.Group className="mb-2 col-xl-12 " controlId="formUser">
                ...
                <Form.Group className="mb-2 col-xl-12 " controlId="formUser">
                  ...
                  <CKEditor
                    initData={<p></p>}
                    onChange={({ editor }) => {
                      //   console.log("Total bytes: " + e.editor.getData());
                      let htmlString = editor.getData();
                      // htmlString = parser.parseFromString(
                      //   htmlString,
                      //   "text/html"
                      // );

                      // htmlString = htmlString.documentElement.innerHTML;
                      setDescription(htmlString);
                    }}
                    onInstanceReady={({ editor }) => {
                      setDescriptionInstance(editor);
                    }}
                  />
                  {/* <Form.Control
                  ...
                  <div className="add-subtask-des">
                    {subTaskArr && subTaskArr.length
                      ? subTaskArr.map((val, index) => {
                        console.log("sdcxvbnmtgdddddddddddddds",val)
                        return (
                          <React.Fragment key={index}>
                            <div className="subtitle-desc">
                              ...
                              <Form.Control
                                ...
                                onChange={(e) => {
                                  let sub = [...subTaskArr];
                                  //console.log(sub,"ddddddddddddddddddddddddd");

                                  sub[index] = {
                                    ...sub[index],
                                    title: e.target.value,
                                    description:
                                      subTaskArr[index].description,
                                  };
                                  // console.log("sub ", sub);
                              ...
                              <CKEditor
                                  // type="classic"
                                  // initData={<p>This is Bliss Mangement</p>}
                                ...
                                "text/html"
                                    ...
                                    htmlString =
                                      htmlString.documentElement.innerHTML;
                                    let sub = [...subTaskArr];
                                   console.log("sub", sub);
                                    sub[index] = {
                                      description: htmlString,
                                   // title: subTaskArr[index].title,
                                    };
                                    console.log("subsss ", sub);
                                    setSubTaskArr([sub]);
                                  }}
                               />
                              {/* <Form.Control
                ...
                <Form.Group className="mb-2 col-xl-12 " controlId="formUser">
                  ...
                  <CKEditor
                    initData={<p>COMMENTS, HISTORY, ATTACHMENTS</p>}
                    onChange={({ editor }) => {
                      //   console.log("Total bytes: " + e.editor.getData());
                      let htmlString = editor.getData();
                      htmlString = parser.parseFromString(
                  ...
                  >
                    {({
                      ...
                      <div className="upload__image-wrapper">
                        <span
                          className="uploadbtn imgdrop_here"
                        ...
                        &nbsp;
                        <span className="uploadbtn" onClick={onImageRemoveAll}>
                          Remove all images
                        </span>
                ...
                {/* <label className="list-right projectpopupsidebar"> */}
                {
                  <>
                    <div className="list-right projectpopupsidebar">
                      ...
                      <div className="sidebar-option">
                        <label>Time Worked on to Date</label>
                        <div className="projectsinbox">
                          <DatePicker
                            dateFormat="MM/dd/yyyy"
                            ClassName="react-date-picker"
