...
import { getRecycleBinItems, recycleBinActions, searchArchiveItems, searchRecycleBinItems } from './../../redux/Action/setting.action';
...
import { listProgramAction } from "../../redux/Action";
...
function RecycleBin() {
  ...
  const deleteHander = async () => {
    if (selection && selection.length) {
      ...
      setPopupMessage([<p>You are about to permenantly delete selected items from the recycle bin.</p>,
  ...
  const restoreHander = async () => {
    if (selection && selection.length) {
      ...
      setPopupMessage([<p>You are about to restore selected items from the recycle bin.</p>, <p> Please confirm this is the action you wish to take</p>])
    }
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(async () => {
      ...
      if (searchTerm !== '') {
        const response = await searchRecycleBinItems(searchTerm);
        if (response.status === 200) {
  ...
  const columns = [
    ...
    { field: '_deletedBy', headerName: 'Deleted By', width: 200 },
  ...
  return (
    <React.Fragment>
      <div className="main-sec main_setting Users_mein recycle">
        {/* <Sidebar sidebarTitle={"Settings"} sidebar={sidebar} /> */}
        <div className="right-sec">
          <div className="top-bar bg-white border-0">
            <Topbar topBarTitle={"Recycle Bin"} />
            <div className="d-flex justify-content-between mt-3">
              <h6 className="d-none d-md-inline-block">Items in your recycle bin will automatically be permanently deleted after 30 days</h6>
            </div>
          </div>
          <div className="right-content-wrap account-form-sec bg-white  h-100 recycle_bin">
